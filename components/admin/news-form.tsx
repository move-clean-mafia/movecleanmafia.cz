...
const NewsForm: React.FC<NewsFormProps> = ({
  ...
  useEffect(() => {
    if (initialData) {
      const publishedAtDate = initialData.publishedAt
        ? initialData.publishedAt instanceof Timestamp
          ? initialData.publishedAt.toDate().toISOString().split('T')[0]
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-gray-50 via-white to-gray-100 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-brand-text">{t('admin.news.loading')}</p>
        </div>
      </div>
    );
  }

  const pageTitle = t('admin.news.editArticle');
  const pageDescription = initialData?.title || t('admin.news.editDescription');
  const submitButtonText = t('admin.news.form.updateArticle');
  const loadingText = t('admin.news.form.updating');
        ...
        <div className="mb-8">
          ...
          <div className="flex items-center gap-4 mb-6">
            ...
            <div>
              ...
              <p className="text-lg text-brand-text">{pageDescription}</p>
            </div>
          </div>
